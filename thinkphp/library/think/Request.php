...
class Request extends \Yaf\Request\Http
{
     ...
     * @access public
     */
    public function __construct(\Swoole\Http\Request $swooleRequest=null)
    {
        $this->init($swooleRequest);
        parent::__construct($this->server['request_uri'], '/');
    ...
    public function init($swooleRequest)
    {
        ...
        if($swooleRequest){
            //$this->method =   $this->setMethod($swooleRequest->server['request_method']);
            $this->header  = $swooleRequest->header?:[];
            $this->server  = $swooleRequest->server;
            //$this->env     = $app->env;
            $inputData     = $this->getInputData($swooleRequest->rawContent());
            $this->get     = $swooleRequest->get?:[];
            $this->post    = $swooleRequest->post ?: $inputData;
            $this->put     = $inputData;
            $this->cookie  = $swooleRequest->cookie?:[];
            $this->file    = $swooleRequest->files?:[];
